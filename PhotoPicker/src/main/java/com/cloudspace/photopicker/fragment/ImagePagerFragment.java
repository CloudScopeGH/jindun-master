...
import com.cloudspace.photopicker.adapter.PhotoPagerAdapter;

public class ImagePagerFragment extends Fragment {

  public final static String ARG_PATH = "PATHS";
  public final static String PATH_TYPE = "PATH_TYPE";
  ...
  private List<Photo> currentPhotos = new ArrayList<>(), selectedPhotos = new ArrayList<>();
  ...
  // type  0:currentPhoto, 1:selectedPhoto
  public static ImagePagerFragment newInstance(PhotoGridAdapter photoGridAdapter, List<String> paths, int type, int currentItem) {

    ImagePagerFragment f = new ImagePagerFragment();

    Bundle args = new Bundle();
    args.putStringArray(ARG_PATH, paths.toArray(new String[paths.size()]));
    args.putInt(PATH_TYPE, type);
    args.putInt(ARG_CURRENT_ITEM, currentItem);
    args.putBoolean(ARG_HAS_ANIM, false);

    f.setArguments(args);

    f.setmPhotoGridAdapter(photoGridAdapter);

    return f;
  }

  // type  0:currentPhoto, 1:selectedPhoto
  public static ImagePagerFragment newInstance(PhotoGridAdapter photoGridAdapter, List<String> paths,int type, int currentItem, int[] screenLocation, int thumbnailWidth, int thumbnailHeight) {

    ImagePagerFragment f = newInstance(photoGridAdapter, paths,type, currentItem);
  ...
  public void setmPhotoGridAdapter(PhotoGridAdapter photoGridAdapter) {
    ...
    currentPhotos.clear();
    currentPhotos.addAll(mPhotoGridAdapter.getCurrentPhotos());
    selectedPhotos.clear();
    selectedPhotos.addAll(mPhotoGridAdapter.getSelectedPhotos());
  }
  ...
  @Override public void onCreate(Bundle savedInstanceState) {
    ...
    if (bundle != null) {
      ...
      path_type       = bundle.getInt(PATH_TYPE);
  ...
  @Nullable @Override public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    View rootView = inflater.inflate(R.layout.photopicker_fragment_image_pager, container, false);
    ...
    mCheckBox = (ImageView) rootView.findViewById(R.id.pager_is_checked);
    initCheckBox(currentItem);
    mCheckBox.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        Photo photo = (Photo) mCheckBox.getTag();
        boolean isChecked = mPhotoGridAdapter.isSelected(photo);
        mPhotoGridAdapter.onPhotoSelected(currentItem, photo, isChecked);
  ...
  private void initCheckBox(int position) {
    List<Photo> currentPhotos;
    if (path_type == 0) {
      currentPhotos = this.currentPhotos;
    } else {
      currentPhotos = this.selectedPhotos;
    }
    if (currentPhotos == null || currentPhotos.size() <= position) return;
    Photo p = currentPhotos.get(position);
    boolean b = mPhotoGridAdapter.isSelected(p);
    mCheckBox.setTag(p);
